...
import { BarChart3, AlertTriangle, TrendingDown, Search, ChevronRight, ChevronDown } from 'lucide-react';
...
export default function RhCargaHorariaPage() {
  ...
  const teto = settingsQuery.data?.teto_ch_semanal ?? 24; // horas/semana — alinhado a hr_settings default
  const rows = workloadQuery.data ?? [];

  const filtered = useMemo(() => {
    const term = q.trim().toLowerCase();
    const list = term
      ? rows.filter((r) => r.nome_completo.toLowerCase().includes(term))
      : rows;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        breadcrumbs={[{ label: 'R.H.', href: '/rh' }, { label: 'Carga Horária' }]}
        title="Carga Horária por Professor"
        description={`Ocupação semanal consolidada a partir da grade horária oficial. Teto configurado: ${teto}h/semana.`}
        icon={BarChart3}
        backTo="/rh"
      />
