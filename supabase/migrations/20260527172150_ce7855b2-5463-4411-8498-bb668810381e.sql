-- 1) Internal function with semester scope ----------------------------------
...
BEGIN
  ...
  -- Bloqueio por histórico: SOMENTE para modelos do escopo selecionado.
  IF array_length(v_class_group_ids,1) > 0 THEN
    ...
    -- Desativa apenas os modelos do escopo (mantém o outro semestre intacto).
    UPDATE weekly_teaching_models wtm
       SET status = 'INACTIVE', updated_at = now()
      FROM (
        SELECT w.id FROM weekly_teaching_models w
        LEFT JOIN subjects s ON s.id = w.subject_id
        WHERE w.class_group_id = ANY(v_class_group_ids)
          AND w.schedule_type = 'CLASS' AND w.status = 'ACTIVE'
          AND (
            v_scope = 'ALL'
            OR s.semester::text = v_scope
            OR s.semester::text = 'ANNUAL'
            OR (w.subject_id IS NULL AND v_scope = 'ALL')
          )
      ) tgt
     WHERE wtm.id = tgt.id;
  END IF;
  ...
  LOOP
    ...
    IF v_scope <> 'ALL' AND array_length(v_subject_set,1) IS NOT NULL THEN
      ...
      IF v_subject_set IS NULL OR array_length(v_subject_set,1) IS NULL THEN
        -- Nada nesta indicação cabe no escopo — pula silenciosamente.
        CONTINUE;
      END IF;
    END IF;
  ...
  -- Recalcula PL apenas quando recriamos do zero (ALL ou SECOND).
  IF v_scope IN ('ALL','SECOND') THEN
    FOR prof IN
  ...
  -- Só desativa vínculos órfãos quando ALL ou SECOND (grade completa).
  IF v_scope IN ('ALL','SECOND') THEN
    UPDATE professor_school_courses
       SET status='INACTIVE', updated_at=now()
  ...
  -- Regenera ocorrências para todos os modelos ATIVOS (idempotente; janela por semestre da disciplina).
...
REVOKE EXECUTE ON FUNCTION public.materialize_grade_from_indications_internal(uuid, text, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.materialize_grade_from_indications_internal(uuid, text, boolean, text) TO authenticated, service_role;

-- 2) Public wrapper with semester scope ---------------------------------------
...
BEGIN
  ...
  v_result := public.materialize_grade_from_indications_internal(p_link_id, p_ano_letivo, p_generate_occurrences, v_scope);
...
REVOKE EXECUTE ON FUNCTION public.materialize_grade_from_indications(uuid, text, boolean, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.materialize_grade_from_indications(uuid, text, boolean, jsonb, text) TO authenticated;