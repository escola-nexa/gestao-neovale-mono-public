CREATE OR REPLACE FUNCTION public.list_school_indication_links()
 RETURNS TABLE(link_id uuid, school_id uuid, school_nome text, qtd_cursos integer, token text, keyword text, is_active boolean, expires_at timestamp with time zone, created_at timestamp with time zone, qtd_indicacoes integer, qtd_turmas integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
