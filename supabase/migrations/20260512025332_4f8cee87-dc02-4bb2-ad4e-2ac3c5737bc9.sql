...
BEGIN
  ...
  -- 2) Idempotência: limpa modelos CLASS e PLANNING existentes para essas turmas/escola
  ...
  END IF;
  -- Limpa PLANNING anterior gerado por este fluxo (escopo: escola desta materialização)
  DELETE FROM weekly_teaching_models
   WHERE school_id = v_school
  ...
  -- 3) Insere weekly_teaching_models CLASS a partir das indicações aprovadas
  FOR r IN
  ...
  -- 3.5) Geração automática de slots de PLANEJAMENTO (1/3 da CH, min 1)
  --      Aloca em horários livres da própria escola onde o professor não tem
  --      conflito com nenhuma outra aula/planejamento ACTIVE (qualquer escola).
  FOR prof IN
  ...
  LOOP
    ...
    -- PL já existente para este professor nesta escola (após o DELETE de PL anteriores)
    ...
    -- Busca slots livres da escola (qualquer dia/horário ACTIVE) que:
    --  (a) não estejam ocupados na própria escola por CLASS/PLANNING ACTIVE
    --  (b) não conflitem com agenda do professor em qualquer escola
    FOR free_slot IN
       ...
       WHERE sts.school_id = v_school
         ...
         AND NOT EXISTS (
           SELECT 1 FROM weekly_teaching_models conf
            WHERE conf.professor_id = prof.professor_id
  ...
  -- 4) Geração opcional de ocorrências anuais (CLASS + PLANNING)
