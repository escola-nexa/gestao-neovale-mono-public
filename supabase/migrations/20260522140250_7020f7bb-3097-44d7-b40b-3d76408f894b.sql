...
BEGIN
  ...
  FOR prof IN
   ...
   LOOP
    ...
    FOR free_slot IN
      -- Aloca PL APENAS no mesmo turno predominante das aulas do professor.
      -- Prioridade: 1) mesmo dia em que o professor já dá aula nesta escola;
      -- 2) slot ainda globalmente livre (evita compartilhar);
      -- 3) dia menos carregado; 4) cedo.
      SELECT sts.id AS slot_id, sts.weekday, sts.start_time, sts.end_time,
       ...
       ORDER BY same_day_rank ASC, occupancy_rank ASC, day_load ASC, sts.weekday, sts.start_time
       LIMIT v_to_create
    LOOP
