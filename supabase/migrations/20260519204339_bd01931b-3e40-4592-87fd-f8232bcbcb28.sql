...
BEGIN
  ...
  SELECT id, start_date, end_date INTO v_calendar_id, v_cal_start, v_cal_end
    FROM academic_calendars WHERE organization_id=v_org AND ano_letivo=p_ano_letivo AND status='ACTIVE' LIMIT 1;
  ...
  LOOP
    ...
    -- ANP conta CH igual à presencial (1 aula = 1 hora cheia), mesmo em slot curto
    ...
    IF v_professor_id IS NOT NULL THEN
      ...
      IF EXISTS (SELECT 1 FROM weekly_teaching_models conf
        ...
        CONTINUE;
      END IF;
      -- CH em outras escolas: ANP conta 1.0h cheia, presencial conta duração real
  ...
  LOOP
    ...
    -- CH total do professor: ANP = 1.0h cheia
  ...
  -- Workload por turno (professor_school_courses): ANP = 1.0h cheia
  FOR bind IN
  ...
  -- Desativa bindings que não foram tocados nesta materialização (turmas do link)
  UPDATE professor_school_courses
