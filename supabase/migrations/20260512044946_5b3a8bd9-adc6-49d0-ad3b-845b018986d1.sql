...
BEGIN
  ...
  LOOP
    INSERT INTO professor_school_courses (organization_id, professor_id, school_id, course_id, status, ch_morning, ch_afternoon, ch_night, bound_by, bound_at)
    VALUES (v_org, bind.professor_id, v_school, bind.course_id, 'ACTIVE', bind.morning_h, bind.afternoon_h, bind.night_h, v_user, now())
    ON CONFLICT (professor_id, school_id, course_id)
    DO UPDATE SET status='ACTIVE',
                  ch_morning=EXCLUDED.ch_morning, ch_afternoon=EXCLUDED.ch_afternoon, ch_night=EXCLUDED.ch_night,
                  unbind_reason=NULL, unbound_at=NULL, unbound_by=NULL, updated_at=now();
