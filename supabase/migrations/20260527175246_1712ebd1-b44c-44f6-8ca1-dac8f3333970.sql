-- Fix: queries de academic_calendars / academic_bimesters estavam usando colunas inexistentes (school_id, ano_letivo, numero).
-- Corrige para usar as colunas reais (academic_year/status no calendar e calendar_id/number no bimester).
...
BEGIN
  ...
  -- FIX: academic_calendars usa academic_year (int) + status, não school_id/ano_letivo.
  ...
  -- FIX: academic_bimesters usa calendar_id + number, não school_id/ano_letivo/numero.
  SELECT end_date INTO v_b2_end FROM academic_bimesters
   WHERE calendar_id = v_calendar_id AND number = 2
   LIMIT 1;
