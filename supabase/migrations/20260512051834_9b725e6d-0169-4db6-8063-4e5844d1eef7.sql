...
-- 1) Atualizar a função para também persistir professor_id na indicação após resolver
...
BEGIN
  ...
  LOOP
    ...
    -- Persistir professor_id resolvido na indicação (auditoria)
...
-- 2) Backfill: preencher professor_id em indicações antigas onde for resolvível
WITH resolved AS (
  SELECT hi.id AS indication_id, p.id AS professor_id
    FROM hr_school_indications hi
    JOIN professors p
      ON p.organization_id = hi.organization_id
     AND lower(p.full_name) = lower(coalesce(hi.candidato_nome,''))
     AND p.deleted_at IS NULL
    LEFT JOIN public.profiles pr ON pr.user_id = p.user_id
   WHERE hi.professor_id IS NULL
     AND coalesce(hi.candidato_nome,'') <> ''
     AND (
       lower(coalesce(pr.email,'')) = lower(coalesce(hi.candidato_email,''))
       OR coalesce(p.phone,'') = coalesce(hi.candidato_telefone,'')
     )
)
UPDATE hr_school_indications hi
   SET professor_id = r.professor_id
  FROM resolved r
 WHERE hi.id = r.indication_id;
