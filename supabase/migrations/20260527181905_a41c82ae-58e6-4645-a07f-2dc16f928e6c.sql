...
BEGIN
  ...
  LOOP
    ...
    -- Resolução de professor: primeiro o vínculo já existente; se vazio e há candidato
    -- do banco de talentos, tenta casar por nome+email/telefone (sem cpf, que não existe em talent_pool_candidates).
    ...
    -- FIX: enum professor_status usa valores em maiúsculas (ACTIVE/INACTIVE/ON_LEAVE).
    ...
    -- FIX: class_groups não tem deleted_at nem created_by, status usa minúsculas ('ativo').
    ...
    -- FIX: school_time_slots usa slot_label (não label), status='ACTIVE' (maiúsculas), sem deleted_at nem created_by.
    ...
    FOREACH v_sid IN ARRAY v_subject_set LOOP
      ...
      -- FIX: weekly_teaching_models não tem created_by.
  ...
  -- FIX: professor_school_courses não tem created_by.
  FOR bind IN
