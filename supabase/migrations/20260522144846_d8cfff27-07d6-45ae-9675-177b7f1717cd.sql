...
BEGIN
  ...
  FOR prof IN
    -- PL é SEMPRE semanal: usa CH semanal real (sem ponderar por bimestres
    -- habilitados), alinhado ao cálculo da tela de conferência
    -- (SubjectBimesterFilterDialog: max(1, round(chSemanal/3))).
    SELECT professor_id,
           ROUND(SUM(slot_h)::numeric, 4) AS total_h_eff
      FROM (
   ...
   LOOP
    ...
    FOR free_slot IN
      ...
      SELECT c.weekday, c.start_time, c.end_time,
             ...
             -- Prefer days that are TOTALLY EMPTY at the school (no CLASS at all)
             CASE WHEN EXISTS (
       ...
       -- HARD: PL nunca pode cair em (dia, horário) onde EXISTE CLASS de qualquer
       -- professor na escola — atende à regra "PL nunca no mesmo horário da aula".
       AND NOT EXISTS (
