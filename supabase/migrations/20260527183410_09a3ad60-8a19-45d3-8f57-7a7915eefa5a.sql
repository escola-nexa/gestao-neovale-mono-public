CREATE OR REPLACE FUNCTION public.materialize_grade_from_indications_internal(p_link_id uuid, p_ano_letivo text, p_generate_occurrences boolean DEFAULT false, p_semester_scope text DEFAULT 'ALL'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Anuais geram apenas UMA vez. No escopo SECOND não tocamos em ANNUAL (já criadas no FIRST/ALL).
  ...
  LOOP
    ...
    -- Filtro por escopo: anuais somente em ALL ou FIRST (geradas uma única vez).
    ...
    FOREACH v_sid IN ARRAY v_subject_set LOOP
      ...
      -- Idempotência: se a disciplina (especialmente ANNUAL) já existe ACTIVE, não recria.
      IF EXISTS (
