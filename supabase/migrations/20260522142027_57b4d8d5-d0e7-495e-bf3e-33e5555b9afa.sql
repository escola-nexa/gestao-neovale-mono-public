...
BEGIN
  ...
  LOOP
    ...
    IF v_professor_id IS NOT NULL THEN
      ...
      IF v_prof_status IS DISTINCT FROM 'ativo' OR v_prof_deleted IS NOT NULL THEN
        v_aulas_ignoradas := v_aulas_ignoradas + 1;
        v_motivos := v_motivos || jsonb_build_object('indication_id', r.id, 'reason','professor inativo');
        CONTINUE;
      END IF;
  ...
  FOR prof IN
   ...
   LOOP
     ...
     -- Resolve course_id e class_group_id de referência (obrigatórios no schema).
     -- Usa o curso/turma com maior CH ativa para o professor nesta escola.
