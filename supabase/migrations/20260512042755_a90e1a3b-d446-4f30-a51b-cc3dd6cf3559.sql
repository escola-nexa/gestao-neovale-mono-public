...
-- =====================================================================
-- 1) Correção definitiva do enum em unmaterialize_grade_from_indications
-- =====================================================================
CREATE OR REPLACE FUNCTION public.unmaterialize_grade_from_indications(p_link_id uuid, p_reason text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_class_group_ids uuid[]; v_models_deactivated int := 0;
  v_occ_cancelled int := 0; v_bindings_deactivated int := 0;
BEGIN
  IF v_user IS NULL THEN RAISE EXCEPTION 'Não autenticado'; END IF;
  IF p_reason IS NULL OR length(trim(p_reason)) < 5 THEN
    RAISE EXCEPTION 'Motivo da reversão é obrigatório (mínimo 5 caracteres)';
  END IF;
  ...
  IF NOT public.has_role(v_user, 'admin'::app_role) THEN
    RAISE EXCEPTION 'Somente administradores podem reverter a grade';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(p_link_id::text, 0));

  SELECT array_agg(cg.id) INTO v_class_group_ids
    FROM hr_indication_classes hc
    JOIN class_groups cg ON cg.school_id = v_school AND cg.course_id = hc.course_id AND cg.nome = hc.nome
   WHERE hc.external_link_id = p_link_id;

  UPDATE weekly_teaching_models
     SET status = 'INACTIVE', updated_at = now()
   WHERE school_id = v_school
     AND status = 'ACTIVE'
     AND (class_group_id = ANY(coalesce(v_class_group_ids, ARRAY[]::uuid[]))
          OR (schedule_type='PLANNING' AND class_group_id IS NULL AND subject_id IS NULL));
  GET DIAGNOSTICS v_models_deactivated = ROW_COUNT;

  UPDATE annual_class_occurrences ao
     SET status = 'CANCELLED'::occurrence_status,
         updated_at = now()
   WHERE ao.organization_id = v_org
     AND ao.occurrence_date >= CURRENT_DATE
     AND ao.status = 'SCHEDULED'::occurrence_status
     AND EXISTS (
       SELECT 1 FROM weekly_teaching_models wtm
        WHERE wtm.id = ao.weekly_model_id
          AND wtm.school_id = v_school AND wtm.status = 'INACTIVE');
  GET DIAGNOSTICS v_occ_cancelled = ROW_COUNT;
  ...
  UPDATE external_links
     SET materialized_at = NULL, materialized_by = NULL,
         materialized_ano_letivo = NULL, updated_at = now()
   WHERE id = p_link_id;

  INSERT INTO audit_events (organization_id, user_id, module, action, details)
  VALUES (v_org, v_user, 'rh_links_escolas', 'unmaterialize_grade',
    jsonb_build_object('link_id', p_link_id, 'school_id', v_school,
      'reason', p_reason,
      'models_deactivated', v_models_deactivated,
      'occ_cancelled', v_occ_cancelled,
...
-- =====================================================================
-- 2) Opções de disciplinas (Nome Boletim) por curso do link
-- =====================================================================
...
BEGIN
  ...
  SELECT
    ...
    MAX(CASE WHEN s_sem = 'FIRST'  THEN s_id END) AS first_subject_id,
    MAX(CASE WHEN s_sem = 'SECOND' THEN s_id END) AS second_subject_id,
    MAX(CASE WHEN s_sem = 'ANNUAL' THEN s_id END) AS annual_subject_id,
    bool_or(s_sem = 'FIRST'),
...
REVOKE EXECUTE ON FUNCTION public.get_course_subject_boletim_options(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_course_subject_boletim_options(uuid) TO authenticated;

-- =====================================================================
-- 3) Vincular manualmente a disciplina de uma indicação (por boletim_key)
-- =====================================================================
CREATE OR REPLACE FUNCTION public.set_indication_grade_subject(p_indication_id uuid, p_boletim_key text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  v_org uuid; v_user uuid := auth.uid();
  v_link uuid; v_course uuid; v_grade jsonb;
  v_first uuid; v_second uuid; v_annual uuid; v_boletim text; v_ch int;
  v_main uuid; v_materialized timestamptz;
BEGIN
  IF v_user IS NULL THEN RAISE EXCEPTION 'Não autenticado'; END IF;

  SELECT hi.external_link_id, hi.course_id, hi.candidato_grade
    INTO v_link, v_course, v_grade
    FROM hr_school_indications hi WHERE hi.id = p_indication_id;
  IF v_link IS NULL THEN RAISE EXCEPTION 'Indicação não encontrada'; END IF;
  IF v_course IS NULL THEN RAISE EXCEPTION 'Indicação sem curso'; END IF;

  SELECT organization_id, materialized_at INTO v_org, v_materialized
    FROM external_links WHERE id = v_link;
  IF v_org IS NULL THEN RAISE EXCEPTION 'Link não encontrado'; END IF;
  ...
  IF NOT (public.has_role(v_user,'admin'::app_role)
    ...
    RAISE EXCEPTION 'Sem permissão';
  END IF;
  IF v_materialized IS NOT NULL THEN
    RAISE EXCEPTION 'Grade já materializada — reverta antes de alterar disciplinas';
  END IF;

  IF p_boletim_key IS NULL OR length(trim(p_boletim_key)) = 0 THEN
    RAISE EXCEPTION 'boletim_key obrigatório';
  END IF;

  -- Resolve IDs do grupo (Nome Boletim) dentro do curso da indicação
  SELECT
    MAX(CASE WHEN s.semester::text='FIRST'  THEN s.id END),
    MAX(CASE WHEN s.semester::text='SECOND' THEN s.id END),
    MAX(CASE WHEN s.semester::text='ANNUAL' THEN s.id END),
    MIN(COALESCE(NULLIF(btrim(s.nome_boletim),''), s.nome)),
    MAX(s.carga_horaria_semanal)
    INTO v_first, v_second, v_annual, v_boletim, v_ch
   FROM subjects s
   WHERE s.course_id = v_course
     AND s.organization_id = v_org
     AND s.deleted_at IS NULL
     AND s.status='ativo'
     AND lower(btrim(COALESCE(NULLIF(btrim(s.nome_boletim),''), s.nome))) = lower(btrim(p_boletim_key));

  IF v_boletim IS NULL THEN
    RAISE EXCEPTION 'Disciplina (Nome Boletim) % não encontrada no curso', p_boletim_key;
  END IF;

  -- Para o subject_id principal usamos ANNUAL > FIRST > SECOND
  v_main := COALESCE(v_annual, v_first, v_second);

  v_grade := COALESCE(v_grade, '{}'::jsonb) || jsonb_build_object(
    'subject_id',         v_main,
    'subject_nome',       v_boletim,
    'boletim_nome',       v_boletim,
    'boletim_key',        lower(btrim(p_boletim_key)),
    'first_subject_id',   v_first,
    'second_subject_id',  v_second,
    'annual_subject_id',  v_annual,
    'carga_horaria_semanal', v_ch
  );

  UPDATE hr_school_indications
     SET candidato_grade = v_grade,
         updated_at = now()
...
REVOKE EXECUTE ON FUNCTION public.set_indication_grade_subject(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.set_indication_grade_subject(uuid, text) TO authenticated;

-- =====================================================================
-- 4) materialize_grade_from_indications — duplica modelos por par FIRST/SECOND
-- =====================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Resolve conjunto de disciplinas (suporta par FIRST/SECOND + ANNUAL)
    ...
    IF v_annual IS NOT NULL THEN
      v_subject_set := v_subject_set || v_annual;
    END IF;
    IF v_first IS NOT NULL THEN
      v_subject_set := v_subject_set || v_first;
    END IF;
    IF v_second IS NOT NULL THEN
      v_subject_set := v_subject_set || v_second;
    END IF;
    ...
    -- Valida que cada disciplina pertence ao curso
    ...
    IF v_professor_id IS NULL THEN
      SELECT p.id INTO v_professor_id FROM professors p
       WHERE p.organization_id = v_org AND lower(p.full_name) = lower(coalesce(r.candidato_nome,''))
         AND (lower(coalesce((SELECT pr.email FROM public.profiles pr WHERE pr.user_id = p.user_id LIMIT 1),'')) = lower(coalesce(r.candidato_email,''))
    ...
    IF v_professor_id IS NOT NULL THEN
      ...
      IF v_prof_deleted IS NOT NULL OR coalesce(v_prof_status,'ativo') <> 'ativo' THEN
        v_aulas_ignoradas := v_aulas_ignoradas + 1;
        v_motivos := v_motivos || jsonb_build_object('indication_id', r.id, 'reason','professor inativo/excluído', 'professor_id', v_professor_id);
        CONTINUE;
      END IF;
    ...
    -- Itera sobre as disciplinas do grupo (ANNUAL OU par FIRST+SECOND)
  ...
  FOR prof IN
    SELECT wtm.professor_id, count(*)::int AS class_count
     ...
     WHERE wtm.school_id=v_school AND wtm.schedule_type='CLASS' AND wtm.status='ACTIVE'
       AND wtm.professor_id IS NOT NULL AND wtm.class_group_id = ANY(v_class_group_ids)
  ...
  LOOP
    ...
    FOR free_slot IN
      SELECT sts.id AS slot_id, sts.weekday, sts.start_time, sts.end_time
       ...
       ORDER BY sts.weekday, sts.start_time LIMIT v_to_create
    LOOP
      BEGIN
        INSERT INTO weekly_teaching_models (
          organization_id, professor_id, school_id, course_id, class_group_id, subject_id,
  ...
  LOOP
    ...
    DO UPDATE SET status='ACTIVE',
      ...
      workload_filled_at=now(), unbind_reason=NULL, unbound_at=NULL, unbound_by=NULL;
  ...
  UPDATE external_links SET materialized_at=now(), materialized_by=v_user,
         materialized_ano_letivo=p_ano_letivo, updated_at=now() WHERE id = p_link_id;

  WITH agg AS (
    SELECT wtm.professor_id,
           sum(CASE WHEN wtm.schedule_type='CLASS' THEN 1 ELSE 0 END) AS aulas,
           sum(CASE WHEN wtm.schedule_type='PLANNING' THEN 1 ELSE 0 END) AS pl,
           sum(CASE WHEN wtm.class_mode='ANP' THEN 1 ELSE 0 END) AS anp,
           ROUND(SUM(EXTRACT(EPOCH FROM (wtm.end_time-wtm.start_time))/3600.0)::numeric,2) AS horas
      FROM weekly_teaching_models wtm
     WHERE wtm.school_id=v_school AND wtm.status='ACTIVE'
       AND (wtm.id=ANY(v_model_ids) OR wtm.id=ANY(v_planning_model_ids))
     GROUP BY wtm.professor_id)
  SELECT jsonb_agg(jsonb_build_object('professor_id',professor_id,'aulas',aulas,'pl',pl,'anp',anp,'horas',horas))
    INTO v_per_prof FROM agg;

  INSERT INTO audit_events (organization_id, user_id, module, action, details)
  VALUES (v_org, v_user, 'rh_links_escolas', 'materialize_grade',
    jsonb_build_object('link_id',p_link_id,'school_id',v_school,'school_name',v_school_name,
      'ano_letivo',p_ano_letivo,'classes_upserted',v_classes_upserted,'slots_upserted',v_slots_upserted,
      'models_upserted',v_models_upserted,'aulas_anp',v_aulas_anp,'aulas_ignoradas',v_aulas_ignoradas,
      'planning_created',v_planning_created,'planning_deficit',v_planning_deficit,
      'bindings_upserted',v_bindings_upserted,'bindings_deactivated',v_bindings_deactivated,
      'notifications_sent',v_notifications_sent,'occurrences_created',v_occurrences_created,
      'teto_ch_semanal',v_teto,
      'shift_morning_end',v_shift_morning_end::text,'shift_afternoon_end',v_shift_afternoon_end::text,
      'per_professor', COALESCE(v_per_prof,'[]'::jsonb)));

  RETURN jsonb_build_object(
    'classes_upserted',v_classes_upserted,'slots_upserted',v_slots_upserted,
    'models_upserted',v_models_upserted,'aulas_anp',v_aulas_anp,'aulas_ignoradas',v_aulas_ignoradas,
    'planning_created',v_planning_created,'planning_deficit',v_planning_deficit,
    'bindings_upserted',v_bindings_upserted,'bindings_deactivated',v_bindings_deactivated,
    'notifications_sent',v_notifications_sent,'occurrences_created',v_occurrences_created,
    'motivos',v_motivos,'preplanning_seed',v_preplanning_seed,
    'materialized_at',now(),'ano_letivo',p_ano_letivo,'school_name',v_school_name);
END;
$function$;
...
-- =====================================================================
-- 5) preview_grade_from_indications — aceitar grupo FIRST/SECOND/ANNUAL
-- =====================================================================
    ...
    SELECT hi.id AS indication_id,
           ...
           COALESCE(
             ...
             (SELECT p.id FROM professors p
                WHERE p.organization_id = v_org
                  AND p.talent_pool_candidate_id = hi.talent_pool_candidate_id LIMIT 1),
             (SELECT p.id FROM professors p
                WHERE p.organization_id = v_org
