-- Drop overload incorreto criado na correção anterior
DROP FUNCTION IF EXISTS public.materialize_grade_from_indications(uuid, integer);
...
BEGIN
  ...
  -- 2) Idempotência — FIX: teacher_plannings não tem deleted_at; remove esse filtro
  ...
  -- 3.6) Vínculo formal com turnos configuráveis
  FOR bind IN
