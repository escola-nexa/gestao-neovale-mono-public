...
CREATE OR REPLACE FUNCTION public._resolve_class_group_id_for_indication(
  p_org uuid,
  p_school uuid,
  p_course uuid,
  p_ano_letivo text,
...
AS $fn$
  WITH candidates AS (
    ...
    WHERE cg.organization_id = p_org
      AND cg.school_id       = p_school
      AND cg.course_id       = p_course
      AND cg.ano_letivo      = p_ano_letivo
      AND cg.status          = 'ativo'
  ),
  ranked AS (
    SELECT id, nome, turno_norm, key_series,
      ROW_NUMBER() OVER (
        ORDER BY
          -- 1) turma com turno igual ao da indicação vence
          CASE WHEN turno_norm IS NOT DISTINCT FROM lower(p_turno) THEN 0 ELSE 1 END,
          -- 2) prefere a que tem turno preenchido sobre a sem turno
          CASE WHEN turno_norm IS NULL THEN 1 ELSE 0 END,
          -- 3) match exato pelo nome da indicação (preserva legado)
          CASE WHEN nome = p_class_nome THEN 0 ELSE 1 END,
          nome
      ) AS rn
    FROM candidates
    WHERE
      key_series = lower(btrim(regexp_replace(p_class_nome, '\s+', ' ', 'g')))
      OR nome    = p_class_nome
  )
  SELECT id FROM ranked WHERE rn = 1
$fn$;

GRANT EXECUTE ON FUNCTION public._resolve_class_group_id_for_indication(uuid,uuid,uuid,text,text,text)
  TO authenticated, service_role;

-- Substitui a resolução de class_group dentro da função principal de materialização
-- (versão text de 5 args). Só altera os trechos do lookup; mantém regra e contagem.
...
BEGIN
  ...
  FOR r IN
    SELECT hi.id, hi.indication_class_id, hi.course_id, hi.candidato_grade,
           ...
           hc.nome AS class_nome, hc.turno AS class_turno
  ...
  LOOP
    ...
    -- HOTFIX: resolve a turma preferindo a que tem turno bate com a indicação
    -- (evita pegar turma-fantasma duplicada sem sufixo de turno)
    v_class_group_id := public._resolve_class_group_id_for_indication(
      v_org, v_school, r.course_id, p_ano_letivo, r.class_nome, r.class_turno
    );
  ...
  IF v_generate_planning THEN
    FOR prof IN
      SELECT DISTINCT wtm.professor_id,
             COALESCE(p.full_name, 'Professor') AS prof_nome
        FROM weekly_teaching_models wtm
        LEFT JOIN professors p ON p.id = wtm.professor_id
       WHERE wtm.school_id = v_school
         AND wtm.status='ACTIVE' AND wtm.schedule_type='CLASS'
         AND wtm.professor_id IS NOT NULL
    LOOP
      v_prof_nome := prof.prof_nome;

      SELECT COUNT(*)::int INTO v_class_slots_count
        FROM (
          SELECT DISTINCT weekday, start_time, end_time, class_group_id, COALESCE(subject_id::text,'') AS sid
      ...
      v_target := GREATEST(1, ROUND(v_class_slots_count::numeric / 3.0)::int);
      ...
      v_pl_breakdown := v_pl_breakdown || jsonb_build_object(
        'professor_id', prof.professor_id,
        'professor_nome', v_prof_nome,
        'class_slots', v_class_slots_count,
        'target', v_target,
        'already', v_already,
        'created', v_created_now,
