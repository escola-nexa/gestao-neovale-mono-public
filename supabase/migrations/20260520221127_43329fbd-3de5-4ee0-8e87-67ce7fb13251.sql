-- 1) Tabela de filtro disciplina x bimestre por link
CREATE TABLE IF NOT EXISTS public.hr_link_subject_bimester_filter (
  ...
  UNIQUE (external_link_id, subject_id, bimester)
);

CREATE INDEX IF NOT EXISTS idx_hr_link_subj_bim_filter_link ON public.hr_link_subject_bimester_filter(external_link_id);
CREATE INDEX IF NOT EXISTS idx_hr_link_subj_bim_filter_subject ON public.hr_link_subject_bimester_filter(subject_id);

ALTER TABLE public.hr_link_subject_bimester_filter ENABLE ROW LEVEL SECURITY;

CREATE POLICY "manage by admin/coord/rh same org"
ON public.hr_link_subject_bimester_filter
FOR ALL
TO authenticated
USING (
...
CREATE TRIGGER update_hr_link_subject_bimester_filter_updated_at
BEFORE UPDATE ON public.hr_link_subject_bimester_filter
...
CREATE OR REPLACE FUNCTION public.materialize_grade_from_indications(
  ...
  p_subject_bimester_filter jsonb DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 3) Internal: PL ponderado por bimestres habilitados + limpeza de ocorrências
...
BEGIN
  ...
  -- PL = max(1, round(aulas_efetivas/3)) com peso = (bimestres_habilitados/4) por disciplina
  FOR prof IN
    SELECT professor_id,
           ROUND(SUM(class_weight)::numeric, 4) AS class_count_eff
      FROM (
        SELECT DISTINCT wtm.professor_id, wtm.weekday, wtm.start_time, wtm.end_time, wtm.class_group_id, wtm.subject_id,
               COALESCE((
     ...
     HAVING SUM(class_weight) > 0
     ORDER BY SUM(class_weight) DESC
  LOOP
    v_target := GREATEST(1, ROUND(prof.class_count_eff / 3.0))::int;
             ...
             (SELECT count(*) FROM (
                SELECT DISTINCT weekday, start_time, end_time
                 ...
                 WHERE w2.professor_id=prof.professor_id AND w2.status='ACTIVE'
                   AND w2.weekday=sts.weekday) wd) AS day_load,
             CASE
