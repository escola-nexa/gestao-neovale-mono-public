-- Corrige a barreira anti-duplicidade para respeitar a regra operacional:
-- duplicidade real = mesma turma + mesma disciplina + mesmo dia + mesmo tempo.
-- Isso permite pares semestrais FIRST/SECOND no mesmo slot, que ocorrem em períodos letivos diferentes.
  ...
  LOOP
    ...
    LOOP
      BEGIN
        ...
        IF v_model_id IS NOT NULL THEN
          ...
          v_motivos := v_motivos || jsonb_build_object(
            'indication_id', r.id,
            'reason', 'aula já existente para esta turma, disciplina, dia e tempo',
            'subject_id', v_sid
          );
          CONTINUE;
        END IF;
      ...
      EXCEPTION WHEN unique_violation THEN
        ...
        v_motivos := v_motivos || jsonb_build_object(
          'indication_id', r.id,
          'reason', 'aula duplicada ignorada — já existe aula ativa para esta turma, disciplina, dia e tempo',
          'sqlstate', SQLSTATE,
          'subject_id', v_sid
        );
      WHEN OTHERS THEN
    ...
    FOR free_slot IN
      WITH shift_times AS (
        SELECT DISTINCT sts.start_time, sts.end_time,
               CASE
                 WHEN sts.start_time < v_shift_morning_end THEN 'MORNING'
    ...
    IF v_to_create > 0 THEN
      ...
      v_motivos := v_motivos || jsonb_build_object(
        ...
        'reason', CASE WHEN v_pl_failed_no_slot > 0
                       THEN 'PL não criado — falha ao criar slot na escola'
                       ELSE 'PL insuficiente — professor sem horário individual livre (todos os slots já têm CLASS ou PLANNING para ele) ou teto de CH excedido' END,
        'turno_preferencial', v_predominant_shift,
        'target', v_target,
...
REVOKE EXECUTE ON FUNCTION public.materialize_grade_from_indications_internal(uuid, text, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.materialize_grade_from_indications_internal(uuid, text, boolean) TO authenticated, service_role;