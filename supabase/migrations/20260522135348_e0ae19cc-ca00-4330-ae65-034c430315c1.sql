...
BEGIN
  ...
  -- Soma a carga horária (em horas) das aulas CLASS do professor nesta escola,
  -- ponderada pelos bimestres habilitados de cada disciplina (B1..B4) — ANP conta como 1h cheia.
  FOR prof IN
    SELECT professor_id,
   ...
   LOOP
    ...
    FOR free_slot IN
      -- Aloca PL em qualquer horário do mesmo período (turno) das aulas do professor
      -- onde ELE não tenha conflito — não exige que o school_time_slot esteja
      -- globalmente vazio (outro professor pode ter aula no mesmo horário).
      -- Ordenação: 1) mesmo turno predominante; 2) slot ainda livre globalmente
      -- (preferimos não compartilhar); 3) dia menos carregado; 4) cedo.
      SELECT sts.id AS slot_id, sts.weekday, sts.start_time, sts.end_time,
       ...
       ORDER BY shift_rank ASC, occupancy_rank ASC, day_load ASC, sts.weekday, sts.start_time
       LIMIT v_to_create
    LOOP
