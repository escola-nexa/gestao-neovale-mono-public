...
ALTER TABLE public.teacher_substitution_requests
  ADD COLUMN IF NOT EXISTS substitute_pix text;
...
CREATE OR REPLACE FUNCTION public.tsr_rh_return(
  ...
  p_substitute_pix text DEFAULT NULL,
...
BEGIN
  ...
  ELSIF p_substitute_talent_pool_candidate_id IS NOT NULL THEN
    SELECT COALESCE(v_name, full_name),
           ...
           COALESCE(v_pix, pix)
      INTO v_name, v_phone, v_email, v_pix
  ...
  END IF;
  IF v_cpf IS NULL OR length(btrim(v_cpf)) = 0 THEN
    RAISE EXCEPTION 'CPF do substituto é obrigatório';
  END IF;
  IF v_phone IS NULL OR length(btrim(v_phone)) = 0 THEN
    RAISE EXCEPTION 'Telefone do substituto é obrigatório';
  END IF;
  IF v_pix IS NULL OR length(btrim(v_pix)) = 0 THEN
    RAISE EXCEPTION 'Chave PIX do substituto é obrigatória';
  END IF;
