   ...
   LOOP
    ...
    FOR free_slot IN
      ...
      SELECT c.weekday, c.start_time, c.end_time,
             ...
             -- Prefer days that are TOTALLY EMPTY of CLASS in the whole school (0 first)
             CASE WHEN EXISTS (
                    SELECT 1 FROM weekly_teaching_models wd3
                     WHERE wd3.school_id=v_school AND wd3.status='ACTIVE'
                       AND wd3.schedule_type='CLASS'
                       AND wd3.weekday=c.weekday
       ...
       WHERE NOT EXISTS (SELECT 1 FROM weekly_teaching_models cf
         ...
         -- HARD RULE: PL never overlaps any CLASS in the school (any professor, any class group)
         AND NOT EXISTS (SELECT 1 FROM weekly_teaching_models oc
    ...
    LOOP
      ...
      IF v_new_slot_id IS NULL THEN
        BEGIN
          ...
          VALUES (
            v_org, v_school, free_slot.weekday,
            COALESCE((SELECT MAX(slot_number)+1 FROM school_time_slots WHERE school_id=v_school AND weekday=free_slot.weekday AND status='ACTIVE'),1),
      ...
      BEGIN
        INSERT INTO weekly_teaching_models (
          organization_id, professor_id, school_id, course_id, class_group_id, subject_id,
          weekday, start_time, end_time, status, schedule_type, school_time_slot_id, class_mode)
        VALUES (v_org, prof.professor_id, v_school, v_pl_course_id, v_pl_class_group_id, NULL,
