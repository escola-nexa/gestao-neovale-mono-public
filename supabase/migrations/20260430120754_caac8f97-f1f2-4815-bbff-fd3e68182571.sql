-- 1) Default de teto de carga horária = 24h
ALTER TABLE public.hr_settings
  ALTER COLUMN teto_ch_semanal SET DEFAULT 24;

UPDATE public.hr_settings
   SET teto_ch_semanal = 24
 WHERE teto_ch_semanal = 20;

-- 2) RPC: submit_school_indication_full com nome do diretor + validação de telefone
...
BEGIN
  ...
  -- Cria as indicações (1 por professor) — exige telefone
