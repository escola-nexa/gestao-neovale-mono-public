-- Add max_planning_per_professor setting (default 8)
ALTER TABLE public.hr_settings
  ADD COLUMN IF NOT EXISTS max_planning_per_professor integer NOT NULL DEFAULT 8;

-- Replace materialize_grade_from_indications with improved PLANNING generation rules
...
BEGIN
  ...
  SELECT COALESCE(teto_ch_semanal,24)::numeric,
         ...
         COALESCE(max_planning_per_professor, 8)
    INTO v_teto, v_shift_morning_end, v_shift_afternoon_end, v_max_pl_per_prof
  ...
  v_max_pl_per_prof := COALESCE(v_max_pl_per_prof, 8);
  ...
  -- =============================================================
  -- 🆕 BLOCO PL — geração com regras A1-A3, B1-B3, C1-C2
  -- =============================================================
  FOR prof IN
    SELECT wtm.professor_id, count(*)::int AS class_count
     ...
     ORDER BY count(*) DESC   -- B3: quem tem mais aulas escolhe primeiro
  LOOP
    -- A2: CEIL em vez de ROUND ; C2: limita a v_max_pl_per_prof
    v_target := LEAST(
      v_max_pl_per_prof,
      GREATEST(1, CEIL(prof.class_count::numeric / 3.0))::int
    );
    ...
    -- B1: calcular turno predominante (manhã/tarde/noite) do professor na escola
    SELECT CASE
    ...
    -- A1: CH atual do professor (CLASS + PLANNING) em todas as escolas
    ...
    FOR free_slot IN
      SELECT sts.id AS slot_id, sts.weekday, sts.start_time, sts.end_time,
             ...
             -- B2: ranquear pelos weekdays menos ocupados deste professor
             ...
             -- B1: prioridade pelo turno predominante
             CASE
       ...
       WHERE sts.school_id=v_school AND sts.status='ACTIVE'
         -- A3: nenhum modelo ACTIVE (CLASS ou PLANNING) nessa escola naquele slot
         ...
         -- A3: professor não pode ter conflito em NENHUMA escola (CLASS ou PLANNING)
       ...
       LIMIT v_to_create
    LOOP
      -- A1: validar teto CH antes de inserir
      IF v_prof_total_ch + free_slot.slot_h > v_teto THEN
        ...
        EXIT;  -- não adianta tentar outros slots — todos somariam ao mesmo prof
      END IF;
      BEGIN
        INSERT INTO weekly_teaching_models (
          organization_id, professor_id, school_id, course_id, class_group_id, subject_id,
