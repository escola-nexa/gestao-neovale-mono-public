-- 1) Helper: classify UCP from subject name (mirrors src/features/rh/lib/classifyUCP.ts)
CREATE OR REPLACE FUNCTION public.classify_ucp(p_name text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_name IS NULL THEN 'OUTRA'
    WHEN p_name ~* '\mUCP\s*III\M' THEN 'UCP3'
    WHEN p_name ~* '\mUCP\s*II\M' THEN 'UCP2'
    WHEN p_name ~* '\mUCP\s*I\M'  THEN 'UCP1'
    WHEN p_name ~* '(empresa\s+pedag|pedag[oó]gica)' THEN 'PEDAGOGICA'
    ELSE 'OUTRA'
  END;
$$;

-- 2) Replace internal materialize function to expand UCP pairs across semesters
...
BEGIN
  ...
  LOOP
    ...
    -- Expansão UCP: inclui automaticamente a disciplina pareada no semestre oposto
    -- (mesma UCP no mesmo curso). Garante que UCP I/II/III do 2º semestre também
    -- recebam weekly_teaching_models quando o diretor indicou apenas o 1º semestre
    -- (ou vice-versa).
    IF array_length(v_subject_set,1) IS NOT NULL THEN
      ...
      FROM (
        ...
        SELECT s2.id
          ...
          -- só pareia se a disciplina pareada estiver no filtro de bimestre deste link
          WHERE EXISTS (
  ...
  -- PL por professor: cota baseada em horários únicos (slot-hours) — coerente
  -- com max(1, round(CH/3)) sem inflar quando pares UCP S1/S2 ocupam o mesmo slot.
  FOR prof IN
   ...
   LOOP
     ...
     IF v_pl_course_id IS NULL THEN
       v_motivos := v_motivos || jsonb_build_object(
         ...
         'reason','PL não criado — sem curso de referência para o professor');
    ...
    FOR free_slot IN
      ...
      SELECT c.weekday, c.start_time, c.end_time,
             ...
             CASE WHEN EXISTS (
               SELECT 1 FROM weekly_teaching_models od
                WHERE od.school_id=v_school AND od.status='ACTIVE'
                  AND od.schedule_type='CLASS' AND od.weekday=c.weekday
             ) THEN 1 ELSE 0 END AS empty_day_rank,
             (SELECT count(*) FROM weekly_teaching_models pp
               WHERE pp.professor_id=prof.professor_id AND pp.school_id=v_school
                 ...
                 AND pp.weekday=c.weekday
             ) AS pl_same_day_rank,
             (SELECT count(*) FROM weekly_teaching_models w3
               WHERE w3.school_id=v_school AND w3.status='ACTIVE'
                 AND w3.schedule_type='PLANNING'
                 AND w3.weekday=c.weekday AND w3.start_time=c.start_time AND w3.end_time=c.end_time
             ) AS occupancy_rank
        FROM candidates c
       WHERE NOT EXISTS (
       ...
       AND NOT EXISTS (
         SELECT 1 FROM weekly_teaching_models oc
          WHERE oc.school_id=v_school AND oc.status='ACTIVE'
            AND oc.schedule_type='CLASS'
            AND oc.weekday=c.weekday
            AND oc.start_time < c.end_time AND oc.end_time > c.start_time
       )
       ORDER BY pl_same_day_rank DESC, empty_day_rank ASC, occupancy_rank ASC, day_load ASC, c.weekday, c.start_time
    LOOP
    ...
    IF v_to_create > 0 THEN
      ...
      v_motivos := v_motivos || jsonb_build_object(
        ...
        'reason', CASE WHEN v_pl_failed_no_slot > 0
                       THEN 'PL não criado — verifique motivos detalhados acima'
                       ELSE 'PL insuficiente — sem horário livre no turno ' || v_shift_label END,
  ...
  SELECT COUNT(DISTINCT school_time_slot_id)::int INTO v_total_slots
   ...
   WHERE school_id = v_school AND status='ACTIVE'
     ...
     AND (id = ANY(v_model_ids) OR id = ANY(v_planning_model_ids));
