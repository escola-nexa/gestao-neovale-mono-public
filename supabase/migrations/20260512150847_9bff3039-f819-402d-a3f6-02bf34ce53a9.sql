...
DECLARE
  ...
  v_per_prof_slots jsonb := '{}'::jsonb;  -- slots únicos staged por professor (chave: weekday|start|end)
  v_slot_key text;
...
BEGIN
  ...
  LOOP
    ...
    IF v_professor_id IS NOT NULL THEN
      ...
      IF EXISTS (SELECT 1 FROM weekly_teaching_models conf
        ...
        CONTINUE;
      END IF;
      -- CH em outras escolas: usa SLOTS ÚNICOS (não duplica por disciplina anual+1sem+2sem no mesmo horário)
      SELECT COALESCE(SUM(slot_h),0)::numeric INTO v_total_h
        FROM (
          SELECT DISTINCT weekday, start_time, end_time, school_id, class_group_id,
                 EXTRACT(EPOCH FROM (end_time-start_time))/3600.0 AS slot_h
      ...
      -- Soma horas staged dessa rodada — apenas UMA vez por slot único
  ...
  -- IMPORTANTE: conta SLOTS ÚNICOS (não múltiplas disciplinas no mesmo horário)
  -- Sem teto fixo. Alinha com src/features/grade-horaria/utils/planningRule.ts
  -- =============================================================
  FOR prof IN
    SELECT professor_id, count(*)::int AS class_count
      FROM (
        SELECT DISTINCT wtm.professor_id, wtm.weekday, wtm.start_time, wtm.end_time, wtm.class_group_id
  ...
  LOOP
    ...
    -- CH total do professor: também por slots únicos
    SELECT COALESCE(SUM(EXTRACT(EPOCH FROM (end_time-start_time))/3600.0),0)::numeric
      INTO v_prof_total_ch
      FROM (
        SELECT DISTINCT weekday, start_time, end_time, school_id, class_group_id, schedule_type
          FROM weekly_teaching_models
  ...
  FOR bind IN
    SELECT professor_id, course_id,
      ...
      FROM (
        SELECT DISTINCT wtm.professor_id, wtm.course_id, wtm.weekday, wtm.start_time, wtm.end_time, wtm.class_group_id,
               EXTRACT(EPOCH FROM (wtm.end_time-wtm.start_time))/3600.0 AS slot_h
          FROM weekly_teaching_models wtm
