...
DECLARE
  ...
  v_active_pairs jsonb := '[]'::jsonb; -- pares professor/curso ativos
...
BEGIN
  ...
  -- Teto de carga semanal (padrão 24h)
  SELECT COALESCE(teto_ch_semanal, 24)::numeric INTO v_teto
  ...
  IF v_calendar_id IS NOT NULL THEN
    v_cal_mid := v_cal_start + ((v_cal_end - v_cal_start) / 2);
  END IF;
  ...
  LOOP
    SELECT id INTO v_class_group_id
     ...
     WHERE school_id = v_school AND course_id = r.course_id
       AND nome = r.nome AND ano_letivo = p_ano_letivo
     LIMIT 1;
  ...
  LOOP
    ...
    IF v_professor_id IS NULL AND r.talent_pool_candidate_id IS NOT NULL THEN
      SELECT id INTO v_professor_id FROM professors
       WHERE talent_pool_candidate_id = r.talent_pool_candidate_id
         AND organization_id = v_org LIMIT 1;
    END IF;
    ...
    -- #1: Conflito entre escolas (mesmo professor já tem CLASS/PLANNING em outra escola no mesmo horário)
    IF v_professor_id IS NOT NULL THEN
      IF EXISTS (
        SELECT 1 FROM weekly_teaching_models conf
         WHERE conf.professor_id = v_professor_id
           AND conf.status = 'ACTIVE'
           AND conf.school_id <> v_school
           AND conf.weekday = v_weekday
           AND conf.start_time < v_end
           AND conf.end_time > v_start
      ) THEN
        v_aulas_ignoradas := v_aulas_ignoradas + 1;
        v_motivos := v_motivos || jsonb_build_object('indication_id', r.id, 'reason', 'conflito com aula em outra escola', 'professor_id', v_professor_id, 'weekday', v_weekday::text, 'label', v_label);
        CONTINUE;
      END IF;
    END IF;

    -- #3: Teto de carga semanal
    IF v_professor_id IS NOT NULL THEN
      ...
      IF v_total_h > v_teto THEN
        v_aulas_ignoradas := v_aulas_ignoradas + 1;
        v_motivos := v_motivos || jsonb_build_object('indication_id', r.id, 'reason', 'excede teto de carga semanal', 'professor_id', v_professor_id, 'teto', v_teto, 'total_horas', v_total_h);
        CONTINUE;
      END IF;
    ...
    IF v_slot_id IS NULL THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        ...
        IF v_slot_id IS NULL THEN
          ...
          CONTINUE;
        ELSE
          v_motivos := v_motivos || jsonb_build_object('indication_id', r.id, 'info', 'slot_reused', 'label', v_label);
        END IF;
      END;
    END IF;
  ...
  LOOP
         ...
         AND NOT EXISTS (
           SELECT 1 FROM weekly_teaching_models occ
            WHERE occ.school_id = v_school AND occ.status = 'ACTIVE'
              ...
              AND occ.start_time = sts.start_time AND occ.end_time = sts.end_time)
         AND NOT EXISTS (
           SELECT 1 FROM weekly_teaching_models conf
            WHERE conf.professor_id = prof.professor_id AND conf.status = 'ACTIVE'
              AND conf.weekday = sts.weekday
              AND conf.start_time < sts.end_time AND conf.end_time > sts.start_time)
    ...
    IF v_to_create > 0 THEN
      ...
      v_motivos := v_motivos || jsonb_build_object(
        'professor_id', prof.professor_id, 'reason', 'horários livres insuficientes para PL',
        'target', v_target, 'criados', v_target - v_to_create);
    END IF;
  END LOOP;

  -- 3.6) Vínculo formal Professor × Escola × Curso
  FOR bind IN
  ...
  LOOP
    ...
    DO UPDATE SET status = 'ACTIVE',
      ...
      unbind_reason = NULL, unbound_at = NULL, unbound_by = NULL;
    v_bindings_upserted := v_bindings_upserted + 1;
  END LOOP;

  -- #10: Desativa vínculos órfãos (professor×curso) que existiam na escola e não foram contemplados
  UPDATE professor_school_courses psc
     SET status = 'INACTIVE',
         unbind_reason = COALESCE(psc.unbind_reason, 'auto: removido na materialização'),
         unbound_at = COALESCE(psc.unbound_at, now()),
         unbound_by = COALESCE(psc.unbound_by, v_user)
   WHERE psc.school_id = v_school
     AND psc.status = 'ACTIVE'
     AND NOT EXISTS (
       SELECT 1 FROM jsonb_array_elements(v_active_pairs) ap
        WHERE (ap->>'p')::uuid = psc.professor_id
          AND (ap->>'c')::uuid = psc.course_id);
  GET DIAGNOSTICS v_bindings_deactivated = ROW_COUNT;

  -- 4) Ocorrências (com filtro de semestre da disciplina)
  IF p_generate_occurrences AND v_calendar_id IS NOT NULL THEN
    ...
    to_insert AS (
      SELECT v_org AS organization_id, md.id AS weekly_model_id,
             l.event_date AS occurrence_date, md.start_time, md.end_time,
             'SCHEDULED'::text AS status
        FROM model_data md
        JOIN letivo l ON EXTRACT(DOW FROM l.event_date)::int = md.dow
       WHERE md.subj_semester IS NULL
          OR md.subj_semester = 'ANNUAL'
          OR (md.subj_semester = 'FIRST' AND l.event_date <= v_cal_mid)
          OR (md.subj_semester = 'SECOND' AND l.event_date > v_cal_mid))
  ...
  -- #6: Notifica professores
  INSERT INTO notifications (user_id, title, message, type, reference_id)
  SELECT DISTINCT p.user_id,
         'Nova grade horária atribuída',
         'Você recebeu aulas na escola desta materialização. Verifique sua grade.',
         'GENERAL', p_link_id
    FROM weekly_teaching_models wtm
  ...
  UPDATE external_links SET materialized_at = now(), materialized_by = v_user,
         materialized_ano_letivo = p_ano_letivo, updated_at = now()
   WHERE id = p_link_id;

  -- #8 Auditoria detalhada por professor
  WITH agg AS (
  ...
  VALUES (v_org, v_user, 'rh_links_escolas', 'materialize_grade',
    jsonb_build_object(
      'link_id', p_link_id, 'school_id', v_school, 'ano_letivo', p_ano_letivo,
      'classes_upserted', v_classes_upserted, 'slots_upserted', v_slots_upserted,
      'models_upserted', v_models_upserted, 'aulas_anp', v_aulas_anp,
      'aulas_ignoradas', v_aulas_ignoradas, 'planning_created', v_planning_created,
      'planning_deficit', v_planning_deficit, 'bindings_upserted', v_bindings_upserted,
  ...
  RETURN jsonb_build_object(
    ...
    'materialized_at', now(), 'ano_letivo', p_ano_letivo);
END;
$function$;
