CREATE OR REPLACE FUNCTION public.generate_teacher_attendance_monthly_sheets_batch(
  ...
  p_professor_ids uuid[] DEFAULT NULL::uuid[]
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Pré-conta professores que JÁ têm folha do mês (serão pulados sem entrar no loop).
  -- Considera o mesmo recorte de escola/lista de professores aplicado na geração.
  SELECT count(DISTINCT ms.professor_id)
    INTO v_skipped
  FROM public.teacher_attendance_monthly_sheets ms
  WHERE ms.organization_id = p_organization_id
    AND ms.reference_year = p_reference_year
    AND ms.reference_month = p_reference_month
    AND (p_professor_ids IS NULL OR ms.professor_id = ANY(p_professor_ids))
    AND (
      p_school_id IS NULL
      OR EXISTS (
        SELECT 1
        FROM public.weekly_teaching_models wtm
        WHERE wtm.organization_id = p_organization_id
          AND wtm.professor_id = ms.professor_id
          AND wtm.school_id = p_school_id
          AND wtm.status = 'ACTIVE'
      )
    );

  -- Itera APENAS professores ainda sem folha no mês (NOT EXISTS no próprio cursor).
  FOR v_prof_id IN
    ...
    WHERE occ.organization_id = p_organization_id
      ...
      AND NOT EXISTS (
        ...
        FROM public.teacher_attendance_monthly_sheets ms
        WHERE ms.organization_id = p_organization_id
          AND ms.professor_id = wtm.professor_id
          AND ms.reference_year = p_reference_year
          AND ms.reference_month = p_reference_month
      )
  LOOP
    BEGIN
      -- generate_teacher_attendance_sheet já chama recompute + recalc internamente.
      -- Não chamamos recalculate_teacher_attendance_monthly_sheet de novo (evita recálculo duplicado).
      v_sheet_id := public.generate_teacher_attendance_sheet(
        p_organization_id, v_prof_id, p_reference_year, p_reference_month
      );
