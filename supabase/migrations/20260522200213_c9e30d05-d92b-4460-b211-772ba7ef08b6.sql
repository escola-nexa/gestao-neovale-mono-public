-- Corrige a regra de conflito para permitir disciplinas semestrais pareadas
-- no mesmo horário, turma e professor (ex.: UCP do 1º e 2º semestre).
  ...
  IF EXISTS (
    SELECT 1
     ...
     WHERE existing.professor_id = NEW.professor_id
       ...
       AND NOT (
         -- Mesmo professor, mesma turma, mesmo slot e disciplinas diferentes
         -- representam alternância semestral, não conflito real.
...
-- Recria a RPC pública adicionando uma validação prévia.
-- Assim a grade antiga só é desativada depois de confirmar que não há
-- conflitos reais capazes de fazer a nova geração ser ignorada.
...
REVOKE EXECUTE ON FUNCTION public.materialize_grade_from_indications(uuid, text, boolean, jsonb) FROM PUBLIC;
