...
BEGIN
  ...
  FOR prof IN
    -- PL é SEMPRE semanal e alinhado à tela de Conferência
    -- (SubjectBimesterFilterDialog): CH semanal = soma de
    -- subjects.carga_horaria_semanal deduplicada por (professor, subject_id).
    -- Mesma disciplina lecionada em N turmas conta 1× (não N×).
    WITH prof_subjects AS (
      SELECT DISTINCT wtm.professor_id, wtm.subject_id
    ...
    SELECT ps.professor_id,
           ROUND(SUM(COALESCE(s.carga_horaria_semanal, 0))::numeric, 4) AS total_h_eff
      FROM prof_subjects ps
      JOIN subjects s ON s.id = ps.subject_id
     GROUP BY ps.professor_id
    HAVING SUM(COALESCE(s.carga_horaria_semanal, 0)) > 0
     ORDER BY SUM(COALESCE(s.carga_horaria_semanal, 0)) DESC
   LOOP
    ...
    FOR free_slot IN
      ...
      SELECT c.weekday, c.start_time, c.end_time,
       ...
       ORDER BY empty_day_rank ASC, occupancy_rank ASC, day_load ASC, c.weekday, c.start_time
    LOOP
