-- Update RLS policy to allow professors to update DEVOLVIDO and AGUARDANDO_ASSINATURA plannings
...
CREATE POLICY "Professors can update own draft plannings"
  ...
  USING (
    ...
    AND status IN ('DRAFT'::teacher_planning_status, 'REJECTED'::teacher_planning_status, 'DEVOLVIDO'::teacher_planning_status, 'AGUARDANDO_ASSINATURA'::teacher_planning_status)
  );

-- Update audit trigger to handle new status
