...
DECLARE
  ...
  v_skip_reason text;
  v_ch_sum numeric;
  v_ch_first numeric; v_ch_second numeric; v_ch_annual numeric;
BEGIN
  ...
  -- PLANNING: gerar em QUALQUER escopo (FIRST, SECOND, ALL).
  -- PL = max(1, ROUND(CH_semanal_efetiva / 3))
  -- CH_semanal_efetiva = MAX(CH_FIRST, CH_SECOND) + CH_ANNUAL
  -- (pois 1º e 2º semestres NÃO rodam na mesma semana — usar MAX evita inflar PLs ao materializar o 2º depois do 1º)
  FOR prof IN
  ...
  LOOP
    ...
    SELECT
      COALESCE(SUM(s.carga_horaria_semanal) FILTER (WHERE s.semester::text = 'FIRST'),  0),
      COALESCE(SUM(s.carga_horaria_semanal) FILTER (WHERE s.semester::text = 'SECOND'), 0),
      COALESCE(SUM(s.carga_horaria_semanal) FILTER (WHERE s.semester::text NOT IN ('FIRST','SECOND') OR s.semester IS NULL), 0)
      INTO v_ch_first, v_ch_second, v_ch_annual
      FROM (
    ...
    v_ch_sum := GREATEST(v_ch_first, v_ch_second) + v_ch_annual;
    ...
    v_pl_breakdown := v_pl_breakdown || jsonb_build_object(
      ...
      'ch_first', v_ch_first,
      'ch_second', v_ch_second,
      'ch_annual', v_ch_annual,
      'ch_effective', v_ch_sum
    );
  END LOOP;
