CREATE OR REPLACE FUNCTION public.materialize_grade_from_indications_internal(p_link_id uuid, p_ano_letivo text, p_generate_occurrences boolean DEFAULT false, p_semester_scope text DEFAULT 'ALL'::text, p_planning_filter jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_generate_planning THEN
    ...
    LOOP
      ...
      -- Regra oficial: target = max(1, round(CLASS_count / 3)) por professor × escola
  ...
  -- BINDINGS (professor_school_courses)
  FOR bind IN
