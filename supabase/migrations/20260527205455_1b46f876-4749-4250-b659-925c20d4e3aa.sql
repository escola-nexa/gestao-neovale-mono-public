...
CREATE OR REPLACE FUNCTION public.materialize_grade_from_indications_internal(
  ...
  p_planning_filter jsonb DEFAULT NULL::jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_skip_reason text;
  v_ch_sum numeric;
  v_generate_planning boolean := (v_scope IN ('ALL','FIRST'));
  -- novas variáveis para alocação robusta de PL
...
BEGIN
  ...
  SELECT id, start_date, end_date INTO v_calendar_id, v_cal_start, v_cal_end
   ...
   WHERE organization_id = v_org AND academic_year = p_ano_letivo::int AND status = 'ACTIVE'
     ...
     AND (
       ...
       OR EXISTS (
         SELECT 1 FROM subjects s
          WHERE s.id = wtm.subject_id
            AND (
              s.semester::text = v_scope
              OR (v_scope = 'FIRST' AND s.semester::text = 'ANNUAL')
            )
       )
     );

  IF v_existing_with_history > 0 THEN
    RAISE EXCEPTION 'Esta grade já possui % aula(s) concluída(s) no escopo %. Reverta primeiro via unmaterialize_grade ou contate suporte.', v_existing_with_history, v_scope;
  END IF;
  ...
  LOOP
    ...
    IF v_professor_id IS NULL AND r.talent_pool_candidate_id IS NOT NULL THEN
      SELECT p.id INTO v_professor_id
        ...
        JOIN talent_pool_candidates t
         ...
         AND (
           (t.email IS NOT NULL AND pr.email IS NOT NULL AND lower(t.email) = lower(pr.email))
           OR (t.phone IS NOT NULL AND p.phone IS NOT NULL AND regexp_replace(t.phone,'\D','','g') = regexp_replace(p.phone,'\D','','g'))
         )
    ...
    IF v_professor_id IS NULL THEN
      SELECT p.id INTO v_professor_id
       ...
       WHERE p.organization_id = v_org
         ...
         AND (
           lower(coalesce(pr.email,'')) = lower(coalesce(r.candidato_email,''))
    ...
    SELECT status::text, deleted_at, user_id INTO v_prof_status, v_prof_deleted, v_prof_user
      FROM professors WHERE id = v_professor_id;
    IF upper(coalesce(v_prof_status,'')) <> 'ACTIVE' OR v_prof_deleted IS NOT NULL THEN
      v_aulas_ignoradas := v_aulas_ignoradas + 1;
      v_motivos := v_motivos || jsonb_build_object('indication_id', r.id, 'reason','professor inativo ou removido','status',v_prof_status);
      CONTINUE;
    END IF;
    ...
    SELECT id INTO v_slot_id
     ...
     WHERE organization_id=v_org AND school_id=v_school
       AND weekday=v_weekday AND start_time=v_start AND end_time=v_end
       AND status='ACTIVE'
     LIMIT 1;
    IF v_slot_id IS NULL THEN
      INSERT INTO school_time_slots (organization_id, school_id, weekday, slot_number, start_time, end_time, slot_label, status)
      VALUES (v_org, v_school, v_weekday, v_slot_number, v_start, v_end, v_label, 'ACTIVE')
    ...
    FOREACH v_sid IN ARRAY v_subject_set LOOP
      ...
      INSERT INTO weekly_teaching_models (
        ...
        professor_id, subject_id, course_id, class_mode, status
      ) VALUES (
        v_org, v_school, v_class_group_id, v_slot_id,
        v_weekday, v_start, v_end, 'CLASS',
        v_professor_id, v_sid, r.course_id, v_class_mode, 'ACTIVE'
  ...
  -- PLANNING: gerado UMA vez por ano (escopo ALL ou FIRST).
  -- target = max(1, round(CH_anual_total / 3)) — CH anual total das indicações aprovadas.
  IF v_generate_planning THEN
    FOR prof IN
      SELECT professor_id, prof_nome, ch_total
      FROM (
        SELECT
          prof_match.professor_id,
          prof_match.prof_nome,
          SUM(DISTINCT_subj.ch_semanal) AS ch_total
        FROM (
          SELECT DISTINCT
            COALESCE(
              hi.professor_id,
              (SELECT p.id FROM professors p
               ...
               WHERE p.organization_id = v_org
                 ...
                 AND (
                   lower(coalesce(pr.email,'')) = lower(coalesce(hi.candidato_email,''))
                   OR coalesce(p.phone,'') = coalesce(hi.candidato_telefone,'')
                 )
               LIMIT 1)
            ) AS professor_id,
            coalesce(p2.full_name, hi.candidato_nome, 'Professor') AS prof_nome,
            hi.indication_class_id AS turma_id,
            s.id AS subject_id,
            s.carga_horaria_semanal AS ch_semanal
          FROM hr_school_indications hi
          LEFT JOIN professors p2 ON p2.id = hi.professor_id
          CROSS JOIN LATERAL (
            SELECT unnest(ARRAY[
              NULLIF(hi.candidato_grade->>'subject_id','')::uuid,
              NULLIF(hi.candidato_grade->>'first_subject_id','')::uuid,
              NULLIF(hi.candidato_grade->>'second_subject_id','')::uuid,
              NULLIF(hi.candidato_grade->>'annual_subject_id','')::uuid
            ]) AS sid
          ) sids
          JOIN subjects s ON s.id = sids.sid
          WHERE hi.external_link_id = p_link_id
            AND hi.status = 'APROVADA'
            AND hi.candidato_grade IS NOT NULL
            AND hi.indication_class_id IS NOT NULL
        ) prof_match
        CROSS JOIN LATERAL (
          SELECT DISTINCT prof_match.turma_id, prof_match.subject_id, prof_match.ch_semanal
        ) DISTINCT_subj
        WHERE prof_match.professor_id IS NOT NULL
        GROUP BY prof_match.professor_id, prof_match.prof_nome
      ) agg
      WHERE EXISTS (
        SELECT 1 FROM weekly_teaching_models wtm
         WHERE wtm.school_id = v_school
           AND wtm.status='ACTIVE' AND wtm.schedule_type='CLASS'
           AND wtm.professor_id = agg.professor_id
      )
    LOOP
      v_prof_nome := COALESCE(prof.prof_nome, 'Professor');
      v_ch_sum := COALESCE(prof.ch_total, 0);
      ...
      IF v_to_create > 0 THEN
        ...
        -- CH total atual do professor (todas as escolas, todos os tipos ativos)
        SELECT COALESCE(SUM(
          CASE WHEN COALESCE(wtm.class_mode,'PRESENCIAL') = 'ANP' THEN 1.0
               ELSE EXTRACT(EPOCH FROM (wtm.end_time - wtm.start_time))/3600.0 END
        ), 0)
          INTO v_prof_total_ch
          FROM weekly_teaching_models wtm
         WHERE wtm.professor_id = prof.professor_id
           AND wtm.status='ACTIVE';

        -- Turno predominante do professor na escola
        SELECT CASE
                 WHEN m_h >= a_h AND m_h >= n_h THEN 'MORNING'
                 WHEN a_h >= n_h THEN 'AFTERNOON'
                 ELSE 'NIGHT' END
          INTO v_predominant_shift
          FROM (
            SELECT
              SUM(CASE WHEN start_time < v_shift_morning_end THEN 1 ELSE 0 END) AS m_h,
              SUM(CASE WHEN start_time >= v_shift_morning_end AND start_time < v_shift_afternoon_end THEN 1 ELSE 0 END) AS a_h,
                 ...
                 (SELECT count(*) FROM (
                    SELECT DISTINCT weekday, start_time, end_time
                     ...
                     WHERE w2.professor_id=prof.professor_id AND w2.status='ACTIVE'
                       AND w2.weekday=c.weekday) wd2) AS day_load
            FROM candidates c
           WHERE NOT EXISTS (
        ...
        LOOP
          ...
          INSERT INTO weekly_teaching_models (
            ...
            v_org, v_school, v_pl_class_group_id, v_new_slot_id,
        ...
        IF v_to_create > 0 THEN
          ...
          v_skip_reason := CASE
            WHEN v_prof_total_ch >= v_teto THEN 'sem horário livre — teto CH excedido'
            ELSE 'sem janela disponível para PL'
          END;
        END IF;
      END IF;
      ...
      v_pl_breakdown := v_pl_breakdown || jsonb_build_object(
        ...
        'professor_nome', v_prof_nome,
        'target', v_target,
        'created', v_created_now + v_already,
        'skipped_reason', v_skip_reason,
        'ch_annual_total', v_ch_sum
      );
    END LOOP;
  END IF;
  ...
  IF v_scope IN ('ALL','SECOND') THEN
    UPDATE professor_school_courses psc
     ...
     WHERE psc.school_id = v_school AND psc.status = 'ACTIVE'
       AND NOT EXISTS (
         SELECT 1 FROM weekly_teaching_models wtm
          WHERE wtm.professor_id = psc.professor_id
            AND wtm.school_id = v_school AND wtm.course_id = psc.course_id
  ...
  RETURN jsonb_build_object(
    ...
    'planning_skipped_scope', NOT v_generate_planning,
