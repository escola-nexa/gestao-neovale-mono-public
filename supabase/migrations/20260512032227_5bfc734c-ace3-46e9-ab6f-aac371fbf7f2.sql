...
-- ============================================================
-- Onda 5: turnos configuráveis em hr_settings
-- ============================================================
ALTER TABLE public.hr_settings
  ADD COLUMN IF NOT EXISTS shift_morning_end time NOT NULL DEFAULT '12:00',
  ADD COLUMN IF NOT EXISTS shift_afternoon_end time NOT NULL DEFAULT '18:00';

-- ============================================================
-- Onda 3 (#9): índice único anti-duplicidade em weekly_teaching_models
-- (chave: school, weekday, start, end, class_group, subject; ignora PL)
-- ============================================================
CREATE UNIQUE INDEX IF NOT EXISTS weekly_teaching_models_class_unique
  ON public.weekly_teaching_models (school_id, weekday, start_time, end_time, class_group_id, subject_id)
  WHERE schedule_type = 'CLASS' AND status = 'ACTIVE';

-- ============================================================
-- Onda 6: RPC de reversão da materialização
-- Mantém histórico (teacher_plannings, attendance, notas) preservando occurrences.
-- ============================================================
...
BEGIN
  ...
  -- Soft-inactivate modelos CLASS+PLANNING desta escola atribuídos por este link
  ...
  -- Ocorrências futuras viram CANCELLED (preserva passado para BI)
  UPDATE annual_class_occurrences ao
     SET status = 'CANCELLED'::class_occurrence_status,
         updated_at = now()
   WHERE ao.organization_id = v_org
     AND ao.occurrence_date >= CURRENT_DATE
     AND ao.status = 'SCHEDULED'::class_occurrence_status
     AND EXISTS (
  ...
  -- Desativa vínculos professor×curso da escola
...
-- ============================================================
-- Onda 4: refatoração da função principal (lock, soft delete, school name, bimesters reais)
-- ============================================================
...
BEGIN
  ...
  -- Onda 4 (#2): lock concorrente por link
  PERFORM pg_advisory_xact_lock(hashtextextended(p_link_id::text, 0));
  ...
  -- Calendário e bimestres reais (Onda 4 #7)
  ...
  -- 2) Idempotência segura (Onda 4 #3): bloqueia se houver histórico (teacher_plannings)
  IF array_length(v_class_group_ids,1) > 0 THEN
    ...
    -- Soft delete: marca como INACTIVE preservando occurrences/pre_plannings
  ...
  LOOP
    ...
    IF v_professor_id IS NULL AND r.talent_pool_candidate_id IS NOT NULL THEN
      SELECT id INTO v_professor_id FROM professors
       WHERE talent_pool_candidate_id = r.talent_pool_candidate_id AND organization_id = v_org LIMIT 1;
    END IF;
    IF v_professor_id IS NULL THEN
      SELECT p.id INTO v_professor_id FROM professors p
       WHERE p.organization_id = v_org AND lower(p.full_name) = lower(coalesce(r.candidato_nome,''))
         AND (lower(coalesce(p.email,'')) = lower(coalesce(r.candidato_email,''))
      ...
      IF v_professor_id IS NOT NULL THEN
        ...
        IF v_subject_id IS NOT NULL THEN
          v_preplanning_seed := v_preplanning_seed || jsonb_build_object(
            'course_id', r.course_id, 'professor_id', v_professor_id, 'subject_id', v_subject_id);
        END IF;
      END IF;
    EXCEPTION WHEN OTHERS THEN
      v_aulas_ignoradas := v_aulas_ignoradas + 1;
  ...
  -- 3.6) Vínculo formal com turnos configuráveis (Onda 5 #5)
  FOR bind IN
  ...
  -- 4) Ocorrências (com filtro semestral por bimestre real — Onda 4 #7)
  ...
  -- 6) Notificações com nome da escola (Onda 5 #4)
