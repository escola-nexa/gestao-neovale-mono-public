...
-- RLS Policy: System can insert notifications (via authenticated users or triggers)
CREATE POLICY "Authenticated users can insert notifications"
ON public.notifications
FOR INSERT
WITH CHECK (auth.uid() IS NOT NULL);

-- Trigger to update updated_at on notifications (reuse existing function if exists)
...
DROP TRIGGER IF EXISTS orientation_created_notification ON public.orientations;
