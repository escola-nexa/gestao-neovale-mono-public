...
BEGIN
  ...
  -- (que já preservam o outro semestre quando o escopo for FIRST/SECOND).
...
BEGIN
  ...
  v_result := public.materialize_grade_from_indications_internal(p_link_id, p_ano_letivo, p_generate_occurrences, v_scope, p_planning_filter);
...
REVOKE EXECUTE ON FUNCTION public.materialize_grade_from_indications(uuid, text, boolean, jsonb, text, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.materialize_grade_from_indications(uuid, text, boolean, jsonb, text, jsonb) TO authenticated;
