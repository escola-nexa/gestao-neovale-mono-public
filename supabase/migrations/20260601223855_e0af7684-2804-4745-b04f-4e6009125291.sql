  ...
  WITH link_keys AS (
    SELECT
      ...
      lower(btrim(regexp_replace(
        regexp_replace(
          regexp_replace(hic.nome, '\s*-\s*(matutino|vespertino|noturno|integral)\s*$', '', 'gi'),
          '\s*ano\s*', ' ', 'gi'
        ),
        '\s+', ' ', 'g'
      ))) AS key_series
...
GRANT EXECUTE ON FUNCTION public.check_link_grade_completeness(uuid) TO service_role;